...
function myFunction() {
        ...
        fpType = document.getElementById("selectHTType").value;
        fpByteSize = document.getElementById("selectByteType").value;
        path = "FHTData/"+fileType+fpType+fpByteSize+'.csv';
        console.log(path);
    }
...
window.onload=function(){
  ...
  var load_all = function(){    //dropdown change
...
var main = function(corr, label_col, label_row){
  ...
  var scale = d3.scale.linear()
      ...
      .range([0, parseFloat(d3.select("input#zoom")[0][0].value) * 200]);

  d3.select("input#zoom").on("change", function() {
    scale = d3.scale.linear()
      .domain([0, d3.min([50, d3.max([label_col.length, label_row.length, 4])])])
      .range([0, parseFloat(this.value) * 200]);
